// Replace all occurences of leysal with your actual quay.io account.
// Tip: use Replace, you find this in the menu under Edit -> Replace. 
// Replace mob with the prefix you used for creating your hello-world-app repository.
...
"""
      ...
      steps {
        script {
          ...
          sh "/kaniko/executor --dockerfile `pwd`/Dockerfile --context `pwd` --destination=quay.io/leysal/hello-world-app:${env.BUILD_ID}"
        } //container
      } //steps
    } //stage(build)
...
spec:
  containers:
  - name: appy
    image: quay.io/leysal/hello-world-app:${env.BUILD_ID}
      ...
      steps {
        ...
        container('argo-cd-ci-builder') {
          ...
          dir("hello-world-deployment") {
            ...
            sh "cd test && kustomize edit set image quay.io/leysal/hello-world-app:${env.BUILD_ID}"
